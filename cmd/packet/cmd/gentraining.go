...
An optional top-level "date" gives the incident date (MM/DD/YYYY or YYYY-MM-DD, default today): every date field of the generated messages gets it. A message's optional "msgNo" gives its message number instead of letting the tool pick one: a whole number (e.g. "XND-101") for a message with one sender, or just the sequence (e.g. "102") to use with each sender's prefix, as for a "from": -1 message; without a suffix, it gets "P", like the tool's own numbers. The numbers the tool picks skip those, and ignore deleted messages; a number already used by a message in the incident is refused. A message's optional "time" (HH:MM) fills its message time field (and any other required time field, such as a form's prepared time), and Claude keeps the message consistent with it; without one, the time fields are left blank.
